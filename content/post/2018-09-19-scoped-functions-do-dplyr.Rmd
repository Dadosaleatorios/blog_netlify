...
Neste post estamos supondo que você já tenha familiaridade com o pacote `dplyr`. Caso ainda não conheça o pacote, veja o post de [introdução ao dplyr](https://www.dadosaleatorios.com.br/post/introdu%C3%A7%C3%A3o-ao-dplyr/).
...
- `_all`: Realiza as operações para todas as colunas do `tibble`. 

- `_at`: Realiza operações sobre um conjunto de variáveis com nomes específicos ou índices das colunas (assim como fazemos no `select`).

- `_if`: Realiza operações sobre um conjunto de colunas para os quais a comparação com algum predicado é verdadeira.
...
```{r}
iris %>% 
  select_all(toupper) # ou rename_all(toupper)
```
...
```{r}
iris %>% 
  mutate_at(vars(ends_with("Length")), funs(. * 100))
```
...
- http://rstudio-pubs-static.s3.amazonaws.com/367502_a41eeee9a7dd4d919b27267f9cc34237.html
