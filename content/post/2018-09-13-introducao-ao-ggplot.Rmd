...
require(ggmap)
  ...
  Este material tem por objetivo passar uma idéia inicial das funcionalidades do pacote GGPLOT2 na visualização de dados.
...
* Demais ajustes.
  ...
  Segue abaixo modelo de gráfico, utilizando a função do pacote **Base** e utilizando o pacote **GGPLOT2**
...
## Base de dados
  ...
  A base de dados utilizada para exemplificar as funções do pacote é a base **mtcars**, dataset já existente no R, que traz informações de desempenho de 32 carros, do ano de 1974.
...
|smooth (Curva suavizada)         	|  geom_smooth( )    |
...
## Temas
  ...
  Os temas controlam a exibição de todos os elementos que não são de dados da plotagem. Você pode substituir todas as configurações por um tema completo como __theme_bw()__ ou optar por ajustar configurações individuais usando __theme()__ e as funções **element_**. Use __theme_set()__ para modificar o tema ativo, afetando todos os gráficos futuros.
...
## Plots para dados específicos

* Gráficos de densidade em 2D
...
p + stat_density_2d(aes(col = ..level..), h = c(5, .5))
```

* Gráficos de calor (Necessário a intalação do pacote "viridis")
...
O ggplot apresenta várias extensões disponíveis para explorar aspectos mais avançados em termos de visualização de dados:

* gganimate - Para criar gráficos ggplot2 animados.
* ggiraph - Gráficos interativos
* ggrepel - Modifica posição de texto para não sobrepôr
* Entre outros.
